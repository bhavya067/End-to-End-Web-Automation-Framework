...
public class Desktops extends DriverTestCase{

	@Parameters({""})
	@Test(description = "Verify that user is able to navigate to 'Desktops' section in 'Computers'" ,groups = {"Computers"}, priority = 3)
	public void validateDesktopsSectioninComputers() throws InterruptedException {
		...
		logoutFromPlatform();
		getLogger().pass("Successfully navigated to 'Desktops' section in 'Computers' and validated the header");
	}
	
	@Parameters({""})
	@Test(description = "Verify that user is able to navigate to other sections from 'Desktops' section in 'Computers'" ,groups = {"Computers"}, priority = 4)
	public void validateOtherSectionsNavigationFromDesktops() throws InterruptedException {
		getLogger().assignAuthor("Bhavya");
		loginToPlatform();
		commonHelper.verifyElementPresent(booksLocators.booksHeaderBtn, "booksHeaderBtn");	
		commonHelper.click(booksLocators.booksHeaderBtn, "booksHeaderBtn", "Clicking on 'Books' header button");	
		...
		logoutFromPlatform();
		getLogger().pass("Successfully navigated to different sections from 'Desktops' section in 'Computers'.");
	}
