...
public class CameraPhotos extends DriverTestCase{
	private static final CustomLogger log4j = CustomLogger.getLogger(CameraPhotos.class);

	@Parameters({""})
	@Test(description = "Verify that user is able to navigate to 'Camera Photos' section in 'Electronics'" ,groups = {"Electronics"}, priority = 1)
	public void validateCameraPhotosSectioninElectronics() throws InterruptedException {
		...
		logoutFromPlatform();
		getLogger().pass("Successfully navigated to 'Camera Photos' section in 'Electronics' and validated the header");
	}
	
	@Parameters({""})
	@Test(description = "Verify that user is able to navigate to other sections from 'Camera Photos' section in 'Electronics'" ,groups = {"Electronics"}, priority = 2)
	public void validateOtherSectionsNavigationFromCameraPics() throws InterruptedException {
		getLogger().assignAuthor("Bhavya");
		loginToPlatform();
		commonHelper.verifyElementPresent(booksLocators.booksHeaderBtn, "booksHeaderBtn");	
		commonHelper.click(booksLocators.booksHeaderBtn, "booksHeaderBtn", "Clicking on 'Books' header button");	
		...
		logoutFromPlatform();
		getLogger().pass("Successfully navigated to different sections from 'Camera Photos' section in 'Electronics'.");
	}
}